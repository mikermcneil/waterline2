...
function AssociationRule (definition) {
  ...
  // Make `this.relation` non-enumerable
  Object.defineProperty(this, 'relation', { enumerable: false, writable: true });

  // Merge properties into the Relation instance itself,
  // unless they are already defined.
  _.defaultsDeep(this, definition);

  this.attrDef = this.relation.attributes[this.attrName];
  ...
  _.defaults(this, {
    ...
    refresh: function () {
      ...
      console.log('refreshing association rule: `%s.%s`', this.relation.identity,this.attrName);

      // hack for development only:
      if (this.attrName === 'recipients') {
        var junctionIdentity = 'chatperson';
        this.relation.orm.junction(junctionIdentity, {

          // By default, store the junction in the relation's datastore
          datastore: this.relation.datastore
        });
        console.log('identifying junction:', junctionIdentity, this.relation.orm.junction(junctionIdentity).datastore);
      }
    },
     ...
     * ```
     *
     * getChildCriteria() would return a modified/"wrapped" subcriteria
     ...
     * @nosideeffects
     */
    getChildCriteria: function (parentBatchResults, originalChildCriteria) {
