  ...
  getCriteria: function(parentBatchResults, originalChildCriteria) {
    ...
    newCriteria = {
      ...
      where: (function _buildJunctorWhere (){
        ...
        // Now build WHOSE subquery on the `onto` side
        // so that the child records are filtered according
        // to the original SELECT...WHERE clause.
        _where[fkToChildOnJunctor] = {
          min: 1,
          whose: (function _buildWhose (){
            var _whose = originalChildCriteria.where;
            return _whose;
          })()
        };
