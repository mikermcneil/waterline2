...
module.exports = {
  ...
  getChildFilter: function(filteredParentBatchResults, childCriteria, parentCriteria) {
    ...
    return function _childFilter(childBatchResults) {
      ...
      // Build a set of primary key values from the parent batch results.
      // (since in a viaFK association, the child results hold the foreign key)
      //
      // e.g. [1,2,3,4,5,6]
      // var parentPKValues = (function _buildParentPKValues () {
      //   var _x = _.pluck(filteredParentBatchResults, this.parent.primaryKey);
      //   return pruneUndefined(_x);
      // })();

      // // Built child criteria ind child records where:
      // // child[fk] =IN= parents[pk]
