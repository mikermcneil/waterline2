...
module.exports = function interpolate (spec, ctxOverride) {
  ...
  return function _bridge ( /* ...., explicitCallback */ ) {
    ...
    // Enforce usage of this bridge method at runtime
    ...
    var adapterArgs = _.reduce(adapterUsage, function (memo, usageArg, i) {
      ...
      // prescribed meaning.
      //
      // Pass the context entity's database down to the adapter
      if (usageArg === 'Database' && database) {
        memo.push(database);
        return memo;
      }
      // Pass the context entity's model down to the adapter
...
function _isUnsupportedByAdapter(method, adapter) {
  var err;
  if (!adapter) {
    err = new WLUsageError({reason: util.format('The `%s()` adapter method can not be called because no adapter was specified', method )});
    return err;
  }
