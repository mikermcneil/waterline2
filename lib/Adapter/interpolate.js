...
module.exports = function interpolate (spec, ctxOverride) {
  ...
  require('assert')(
    typeof spec === 'object',
    'Invalid `spec` passed to `Adapter.interpolate()`'
  );
  require('assert')(
    ctxOverride === undefined || typeof ctxOverride === 'object',
    'Invalid `ctxOverride` passed to `Adapter.interpolate()`'
  );
  require('assert')(
    typeof spec.method === 'string',
    'Invalid `spec.method` ("'+spec.method+'") passed to `Adapter.interpolate()`- should have been the name of an adapter method, like "find" or "create"'
  );
   ...
   *
   * TODO: (refactor) more comments in this function (it's a little "dense"...)
   *
  ...
  return function _bridge ( /* ...., explicitCallback */ ) {
    ...
    // Look up the live Adapter, Model, and Database instances
    var adapter = ctx.getAdapter();
    var database = ctx.getDatabase && ctx.getDatabase();
    ...
    var adapterArgNames = _.find(adapterUsage, function (args, versionString) {
    ...
    if (!adapterArgNames) {
      return implicitCallback(new WLUsageError(util.format(
        ...
        '\n'+
        _.reduce(adapterUsage, function (memo, args,versionString) {return memo+'• '+versionString+'\n';}, '') +
        '\n'+
    ...
    var adapterArgs = _.reduce(adapterArgNames, function (memo, adapterUsageArg, i) {
      ...
      // prescribed meaning.
      //
      // Pass the Database instance down to the adapter
      if (adapterUsageArg === 'Database') {
        memo.push(database);
        return memo;
      }
      // Pass the database's identity down to the adapter
      else if (adapterUsageArg === 'Database.identity') {
        memo.push(database.identity);
        return memo;
      }
      // Pass the Model instance down to the adapter
      else if (adapterUsageArg === 'Model') {
        memo.push(model);
        return memo;
      }
      // Pass the model's identity down to the adapter
      else if (adapterUsageArg === 'Model.identity') {
      ...
      else if (adapterUsageArg === 'callback') {
        return memo;
      }
      ...
      // position.
      var index = _.findIndex(usage, {label: adapterUsageArg});

      // But skip unmatched arguments (don't pass them down to the adapter)
...
function _talkToAdapter (options) {
  ...
  var deferred = new Deferred({
    ...
    // "Switchbackify" the callback, if a `Switchback` factory was passed in
