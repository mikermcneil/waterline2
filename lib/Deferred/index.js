...
module.exports = function (config) {
  ...
  // `.exec()` again will have no effect, although it will
  // still bind another callback if you pass one.
  ...
  function Deferred (opts, worker) {
    ...
    this.exec = function (exec_cb) {
      ...
      var whenWorkerFinished = function ( /* args sent back from worker */ ) {
        ...
        if (emitter.emit) {
          emitter.emit.apply(emitter, ['finished'].concat(self._workerResults));
        }
        self.empty = true;
        ...
        if (!exec_cb) return;
        var call_exec_cb_withResults = _.partialApply(exec_cb, self._workerResults);
      ...
      if (this.flowing) {
        ...
        // `exec_cb` immediately (well, after one tick)
        if (this.empty) {
        ...
        // to listen for when it _is_, and run `exec_cb` then.
          ...
          emitter.on('finished', run_exec_cb);
          return;
        }
