...
module.exports = function (config) {
  ...
  function Deferred (opts, worker) {
    ...
    if (config.Promise) {
      ...
      // Bluebird-specific:
      if (config.Promise.onPossiblyUnhandledRejection) {
        config.Promise.onPossiblyUnhandledRejection(function(error){
          // Ignore `onPossiblyUnhandledRejection`, since user might
          // just be using `.exec()` syntax
        });
      }
      var promise = new config.Promise(function (accept, reject) {
