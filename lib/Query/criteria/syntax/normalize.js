...
function normalizeWhereTree (whereTree, targetRelation, criteriaMetadata) {
  ...
  return _.reduce(whereTree, function (memo, sub, attrName) {
    ...
    if (sub.whose) {
      ...
      // Try to determine the `subqueryRelation`
      var subqueryRelation;
      if (targetRelation && targetRelation.orm) {
        if (sub.from) {
          subqueryRelation = targetRelation.orm.model(sub.from);
        }
        else if (sub.junction) {
          subqueryRelation = targetRelation.orm.junction(sub.junction);
        }
      }
...
function normalizeSelectTree (selectTree, targetRelation, criteriaMetadata) {
  ...
  //
  if (targetRelation && _.isEqual(selectTree, {}) || selectTree === '*') {
  ...
  return _.reduce(selectTree, function (memo, sub, attrName) {
    ...
    if (targetRelation && targetRelation.orm) {
      if (sub.from) {
        joinRelation = targetRelation.orm.model(sub.from);
      }
      else if (sub.junction) {
        joinRelation = targetRelation.orm.junction(sub.junction);
      }
    }
