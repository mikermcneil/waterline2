...
function criteriaCursor (options, cb) {
  ...
  console.log('Spawned cursor @ level %d', depth, 'from '+criteria.from.entity+ ': '+criteria.from.identity);
  console.log();
  ...
  // original thing that used to be in Query constructor for the top-level Heap:
  // from: this.from,
 ...
 //  }
  ...
  // };
  //
  // var localHeap = new QueryHeap({orm: orm});
      ...
      function finishBatch () {
        ...
        if (previous) {
          ...
          _(doubleFilteredBatchResults).each(function (record){
            console.log('checking for related parent records for', record);
            ...
            _.each(relatedParentRecords, function (potentialParentRecord) {
              ...
              var _bufferIdentity = qpath + '['+potentialParentPKValue+']';
              if (!query.heap.get(_bufferIdentity).length) {
        ...
        // Otherwise, just push the records as "root"
          ...
          query.heap.pushFootprints(qpath, doubleFilteredBatchResults);
        ...
        // TODO:
        // Fix `populate..skip` + `populate..limit` + `populate..sort`
        // Unfortunately, all three of those modifiers must be applied on
        // a per-record basis, not per-batch.  This is tricky.
        // The probable best solution is to utilize more buffers within
        // the heap, e.g.:
        //  • root
        //  • root[0].brothers
        //    • root[0].brothers[0].bestFriend
        //    • root[0].brothers[1].bestFriend
        //    • ...
        //  • root[0].sisters
        //    • root[0].sisters[0].bestFriend
        //    • root[0].sisters[1].bestFriend
        //    • ...
        //  • root[1].brothers
        //    • root[1].brothers[0].bestFriend
        //    • root[1].brothers[1].bestFriend
        //    • ...
        //  • root[1].sisters
        //    • root[1].sisters[0].bestFriend
        //    • root[1].sisters[1].bestFriend
        //    • ...
        //  • root[2].brothers
        //    • root[2].brothers[0].bestFriend
        //    • root[2].brothers[1].bestFriend
        //    • ...
        // etc.

        // Now, before we push these footprint results into the cache, we must group our
        // `doubleFilteredBatchResults` by associated parent record. This is so that
        // populate..limit/skip are appropriately managed on a rer-record basis.
        // var groups = _.reduce();
        // ...
        // qpath + '['+i+']';

        // Now we can safely push the records in `doubleFilteredBatchResults`
        // to the appropriate buffers in our heap for ongoing sorting/pagination.
        // var bufferGroups = _.groupBy(doubleFilteredBatchResults, function (record) {
        //   grandparents
        ...
        // _.each(bufferGroups, function _pushEachSetOfFootprints(records, grandparentPK){

        //   // parentPK ==> the unique primary key value of the ancestral record
        //   // all of these `records` are related to.  It's called `grandparentPK`
        //   // because it's referring to a parent of the `doubleFilteredBatchResults`,
        //   // which are sometimes referred to as "parent" records themselves.
        //   // Hence the gray beard and all that.  At some point we'll figure out
        //   // a better way to name all this stuff.
        //   query.heap.pushFootprints(qpath+'['+grandparentPK+']', records);
        // });
      ...
      async.each(Object.keys(joins), function eachJoin (attrName, next_association) {
        ...
        return criteriaCursor({
          ...
          previous: {
            ...
            getRelated: rule.buildGetRelatedFn(filteredBatchResults),
            rule: rule
          }
        },
  ...
  function whenDoneWithAllBatches (err) {
    ...
    var _bufferRegExp = new RegExp('^'+qpath+'\\[([^\\.\\[\\]]+)\\]$');
    var _bufferIdents;
    if (previous) {
      ...
      where: (function _topFootprintWHEREClause () {
        ...
        where[src.primaryKey] = (function _topFootprintPKValues() {
          ...
          var footprints = _.reduce(_bufferIdents, function _paginateAndSortFootprintsForEachParentRecord (memo, bufferIdent) {
            ...
            //
            // _someFootprints = _someFootprints.slice(criteria.skip||0, (criteria.limit||1000)+(criteria.skip||0) );
    ...
    .exec(function (err, results) {
      ...
      console.log('\n-(at depth === %d, qpath==="%s", from %s: "%s")-\nChecking if there are records to rehydrate...',depth, qpath, criteria.from.entity, criteria.from.identity);
      console.log('-- _bufferIdents', _bufferIdents);
      console.log('                         (will go through %d hydrated results)', results.length);
      console.log('                         (criteria for hydration was:', {
        from: criteria.from,
        ...
        where: (function _topFootprintWHEREClause() {
          ...
          where[src.primaryKey] = (function _topFootprintPKValues() {
            ...
            // Get the union of the footprints from ALL the buffers-- but first, do a final
            // sort/paginate on each buffer to ensure that only the relevant footprints are
            // hydrated.
            var footprints = _.reduce(_bufferIdents, function _paginateAndSortFootprintsForEachParentRecord(memo, bufferIdent) {
              ...
              console.log('Gathering footprints from buffer `'+bufferIdent+'` which has : ',util.inspect(_someFootprints,false,null));
              _someFootprints = WLTransform.sort(_someFootprints, criteria.sort || {});
              console.log('after sorting : ',util.inspect(_someFootprints,false,null));
              ...
              //
              _someFootprints = _someFootprints.slice((criteria.skip || 0), (criteria.limit || 1000) + (criteria.skip || 0));
              console.log('_someFootprints.slice('+(criteria.skip || 0)+', '+((criteria.limit || 1000) + (criteria.skip || 0))+')');
              console.log('after slicing : ',util.inspect(_someFootprints,false,null));

              // ********************************************************************************
              // HERE IS THE ISSUE
              // Slicing these buffers cuts off records that shouldn't be excluded!
              // ********************************************************************************
      ...
      if (!previous) {
        ...
        // Extract primary keys from bufferidents
        // var parentPKVals = _.reduce(_bufferIdents, function (memo, bufferIdent) {
        ...
        // TODO: rule.buildGetRelatedFn

        // TODO: Now after grouping, `rehydrate` each buffer with the proper set of hydrated results.


        // _.each(_bufferIdents, function (bufferIdent) {

        //   // Parse the primary key value of the **parent relation from the previous recursive step**
        //   // from the buffer identity.
        //   var bufferPKValue = (function _parseBufferPKValue () {
        //     var matchedTokens = bufferIdent.match(_bufferRegExp);
        //     return matchedTokens && matchedTokens[1];
        //   })();

        //   // Then push the subset of hydrated results related with the parent record with
        //   // the buffer's primary key value (e.g. the "feb3224b01380a" in `root.foo.bar[feb3224b01380a]`)


        //   var subsetOfResults = _.where(results, function _getSubsetOfHydratedResults (record) {
        //     return record[previous.relation.primaryKey] === bufferPKValue;
        //   });
        //   // console.log('results: ',results);
        //   // console.log('bufferPKValue: ',bufferPKValue);
        //   // console.log('bufferIdent: ',bufferIdent);
        //   // console.log('subsetOfResults: ',subsetOfResults);
        //   heap.rehydrate(bufferIdent, subsetOfResults);
        // });
      }
