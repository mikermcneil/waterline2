    ...
    .exec(function (err, results) {
      ...
      if (!previous) {
        ...
        _.each(_bufferIdents, function (bufferIdent) {
          ...
          // Then push the subset of hydrated results whose primary key value
          // matches the last one in the buffer identity.
          heap.push(bufferIdent, _.reduce(results, function _getSubsetOfHydratedResults (memo, result) {
            if (result[previous.relation.primaryKey] === bufferPKValue) {
              memo.push(result);
            }
            return memo;
          }, []));
