...
var criteriaUnion = require('./syntax/criteria-union'); // TODO: use this for optimizations
  ...
  async.doUntil(function doWhat (next_batch) {
    ...
    .exec(function (err, _batchResults){
      ...
      function finishBatch () {
        ...
        //
        // <optimization>
        // TODO: skip this in-memory filter if the WHERE filter was already
      ...
      async.each(Object.keys(joins), function eachJoin (attrName, next_association) {
        ...
        var subcriteria = {
          ...
          select: (function _buildSELECT(){
            ...
            // (TODO: maybe we can remove this? it should be happening automatically
          ...
          // (i.e. could be top-level `{where:{}}` or nested `{select: {where:{...}}`-- same thing)
          where: {},
