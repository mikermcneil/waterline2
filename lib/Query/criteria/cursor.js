 ...
 //  }
  ...
  function whenDoneWithAllBatches (err) {
    ...
    src.find({
      ...
      where: (function _topFootprintWHEREClause () {
        ...
        where[src.primaryKey] = (function _topFootprintPKValues() {
          ...
          //
          // TODO:
          // Investigate this section and make sure
          // this in-memory filtering really needs to happen...
          //
          var footprints;
          footprints = query.heap.get(qpath);
          ...
          footprints = footprints.slice(criteria.skip||0, (criteria.limit||1000)+(criteria.skip||0) );
          footprints = _.pluck(footprints, src.primaryKey);
          // console.log('some in-memory filtering happened.. and reduced heap's local buffer to:',util.inspect(footprints,false,null));
          return footprints;
        })();
        return where;
      })(),
    ...
    .exec(function (err, results) {
      ...
      // Push finalized results onto the heap
      // console.log('pushing results (%s) to ',util.inspect(results), qpath);
