...
function criteriaCursor (options, cb) {
  ...
  var MIN_NUM_INTERMEDIATE_RESULTS = options.minNumIntermediateResults||2; // TODO: prbly default to 100
  ...
  async.doUntil(function doWhat (next_batch) {
    ...
    .exec(function (err, _batchResults){
      ...
      // Otherwise `filteredBatchResults` is ready to use-- continue forward.

      // </optimization>
      //

      log.skip('**\npage #'+pageNo+'\nfinding from:',modelIdentity,'\nselect:',util.inspect(query.criteria.select, false, null),'\nLimit:'+BATCH_SIZE+'\nSkip:'+(BATCH_SIZE*pageNo));
      log.skip('criteria:', query.criteria);
  ...
  function doUntil () {
    log.skip('checking for more batches of "%s"',modelIdentity, '(batchResults.length < BATCH_SIZE, '+batchResults.length + ' < '+BATCH_SIZE+')');
