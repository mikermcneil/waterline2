...
function criteriaCursor (options, cb) {
  ...
  // Constants for srctypes
  var SRCTYPE = {MODEL: 1, JUNCTION: 2};
  ...
  log.skip('%s :: flatWhere:',srcIdentity, flatWhere);
  ...
  log.skip('FROM=',srcIdentity,'primaryKey:',primaryKey,'starting SELECT:',select);
  ...
  async.doUntil(function doWhat (next_batch) {
    ...
    if (srcType===SRCTYPE.JUNCTION) {
      batchCriteria.junction = junctionIdentity;
    }
    else if (junctionIdentity) {
      batchCriteria.from = modelIdentity;
    }
    log.debug('Executing raw paging query=', batchCriteria);
    ...
    .exec(function (err, _batchResults){
      ...
      // (or use optimized approach, i.e. call `.join()` in the adapter)
        ...
        async.each(Object.keys(joins), function eachJoin (attrName, next_association) {
          ...
          src.associations=src.associations||{};
          var assoc = src.associations[attrName];
          if (!assoc) {
            return next_association(new WLUsageError(util.format('Query failed - "%s" model has no "%s" association',srcIdentity, attrName)));
          }
          ...
          if (assoc.type === '1-->N') {
            ...
            doRecursiveStep(subop, function  memfilter(subResults) {
              ...
              log.skip('filteredBatchResults (%s)', srcIdentity, _.pluck(filteredBatchResults, primaryKey));
            ...
            log.skip('looking up %s in assoc\'s foreign keys:',srcIdentity, assoc.foreignKeys);
            log.skip('filteredBatchResults',filteredBatchResults);
            log.skip('plucked "%s" to get invalues:: %s',assoc.foreignKeys[srcIdentity], inValues);

            // N.1 :: Foreign key is on child model.
            // Have subop find child records where:
            // child[fk] =IN= parents[pk]
            subop.where[assoc.foreignKeys[srcIdentity]] = inValues;
            ...
            doRecursiveStep(subop, function memfilter (subResults) {
              ...
              var childPK = otherModel.primaryKey;
              var childFK = assoc.foreignKeys[srcIdentity];
              var subquery = subqueries[attrName];
              ...
              log.skip('final filtered subresults (%s.%s)', srcIdentity, attrName, _.pluck(subResults, otherModel.primaryKey));
          ...
          function doRecursiveStep (subop, filter, doRecursiveStep_cb) {
            ...
            log.skip('\n~~~ STARTING NEW RECURSIVE STEP ~~~\n(%s.%s --> %s)\n-----\n%s\n-----\n',srcIdentity, attrName, otherModel.identity, util.inspect(subop));
            // Note:
            ...
            criteriaCursor({
              ...
              log.skip('Does %s.%s have a related subquery?', srcIdentity, attrName, hasSubquery,'::',subquery);
              ...
              if ( !hasSubquery ) {
                ...
                log.skip('pushed %d "%s"s to the local query heap', filteredBatchResults.length, srcIdentity);
                log.skip('--  /  --');
                // localQueryHeap.push(srcIdentity, filteredBatchResults);
              }
              ...
              // remove extra entries)
                ...
                log.skip('filtered subop (%s.%s) results', srcIdentity, attrName, filteredSubopResults);
                ...
                if (assoc.type === '1-->N') {
                  ...
                  log.skip('ACTUALLLY PERFORMING BACKFILTER ON N<->1 (%s.%s)', srcIdentity, attrName);
                  ...
                  backfilteredBatchResults = _.where(filteredBatchResults, function (parentResult) {
                    ...
                    var subopFKs = _.pluck(filteredSubopResults, assoc.foreignKeys[srcIdentity]);
                ...
                log.skip('pushed %d "%s"s to the local query heap', backfilteredBatchResults.length, srcIdentity);
                // localQueryHeap.push(srcIdentity, backfilteredBatchResults);
                log.skip('--  /  --');
                log.skip('pushed:\n',backfilteredBatchResults,' to subop\'s ('+subop.from+') local QC as "'+srcIdentity+'" `toOmit`:', localQueryHeap);
              } // </if>
        ...
        function whenDoneWithAsyncEach (err) {
          log.skip('done with batch of %s', srcIdentity);
  ...
  function doUntil () {
    log.skip('checking for more batches of "%s"',srcIdentity, '(batchResults.length < BATCH_SIZE, '+batchResults.length + ' < '+BATCH_SIZE+')');
  ...
  function afterwards(err) {
    ...
    if (srcType===SRCTYPE.JUNCTION) {
      inCriteria.junction = srcIdentity;
    }
    else {
      inCriteria.from = srcIdentity;
    }
    ...
    .exec(function (err, results) {
      ...
      // heap.wipe(srcIdentity);

      // Push results onto the QueryHeap
      heap.push(src, results);
      log.skip('pushed %d "%s"s to heap:',heap.get(src).length, srcIdentity, results);
