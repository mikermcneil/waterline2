...
function criteriaCursor (options, cb) {
  ...
  log.skip('Running cursor on "%s"', srcIdentity,'for criteria:',criteria);
  if (!src) {
    ...
    .exec(function (err, _batchResults){
      ...
      // (or use optimized approach, i.e. call `.join()` in the adapter)
        ...
        async.each(Object.keys(joins), function eachJoin (attrName, next_association) {
          ...
          if (!rule) {
            return next_association(new WLUsageError(util.format('Query failed - "%s" model has no association "%s" association rule',srcIdentity, attrName)));
          }
          ...
          //   subcriteria.where[assoc.foreignKeys[srcIdentity]] = inValues;
          ...
          //     log.skip('final filtered subresults (%s.%s)', srcIdentity, attrName, _.pluck(subResults, otherRelation.primaryKey));
          ...
          function doRecursiveStep (subcriteria, filter, doRecursiveStep_cb) {
            ...
            log.skip('\n~~~ STARTING NEW RECURSIVE STEP ~~~\n(%s.%s --> %s)\n-----\n%s\n-----\n',srcIdentity, attrName, otherRelation.identity, util.inspect(subcriteria));
            // Note:
            ...
            criteriaCursor({
              ...
              // remove extra entries)
                ...
                log.skip('filtered subcriteria (%s.%s) results', srcIdentity, attrName, filteredSubcriteriaResults);
                ...
                if (assoc.type === '1-->N') {
                  ...
                  backfilteredBatchResults = _.where(filteredBatchResults, function (parentResult) {
                    ...
                    var subcriteriaFKs = _.pluck(filteredSubcriteriaResults, assoc.foreignKeys[srcIdentity]);
                ...
                log.skip('--  /  --');
                log.skip('pushed:\n',backfilteredBatchResults,' to subcriteria\'s ('+subcriteria.from+') local QC as "'+srcIdentity+'" `toOmit`:', localQueryHeap);
              } // </if>
