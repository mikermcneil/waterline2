  ...
  async.doUntil(function doWhat (next_batch) {
    ...
    .exec(function (err, _batchResults){
      ...
      if (
        ...
        // and there aren't enough `_batchResults` to be "worth it" to continue
        // with the recursive step,
        _filteredBatchResults.length < MIN_NUM_INTERMEDIATE_RESULTS
      ) {
        // get another page of results before continuing.
