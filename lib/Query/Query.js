...
var $$ = require('./syntax/MODIFIERS');
var keysNotIn = require('./syntax/keysNotIn');
 ...
 * required- for instance it might be spawned directly from a Datastore.
 ...
 * mentioned previously- this is usually either a Model or Datastore.)
...
function Query (opts, _adapterWorker) {
  ...
  if (typeof opts.update === 'undefined') {
    opts.update = _.pick(opts, keysNotIn($$.CRITERIA_MODS) );
  }
  ...
  // in WL1 integration:
  opts.criteria = _.reduce(opts.criteria, function (memo, val, key) {
  ...
  if (
    ...
    opts.criteria.max ||
    opts.criteria.min ||
    opts.criteria.average ||
    opts.criteria.groupBy
  ) {
    opts.raw = true;
  }
  ...
  var targetModel;
  if (opts.criteria.from && opts.orm) {
    targetModel = opts.orm.model(opts.criteria.from);
  }
  //
  ...
  opts.criteria = normalizeCriteria(opts.criteria, targetModel, flags);
  ...
  //   // Default values for the initial criteria tree.
  //   criteria: {
  //     select: {},
  //     where: {},
  //     limit: 30,
