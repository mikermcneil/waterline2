...
var normalizeCriteria = require('root-require')('standalone/normalize-criteria');
...
function Query (opts, _adapterWorker) {
  ...
  // (NOTE: this has the side-effect of mutating `criteriaMetadata`!)
  opts.criteria = normalizeCriteria(opts.criteria, targetRelation, criteriaMetadata);

  // Examine criteriaMetadata and set the appropriate metadata properties on `opts`
 ...
 * @return {Query}
 */
Query.prototype.run = require('./Query.prototype.run');
 ...
 *
 * Since this method works w/ arrays, it can be
 * called on an array of results, a single result
 ...
 * @api private
 */
Query.prototype.parseResults = function _ParseQueryResults (results) {
  return results;
};
