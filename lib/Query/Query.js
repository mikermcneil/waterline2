...
function Query (opts, _adapterWorker) {
  ...
  if (opts.criteria.from && opts.orm) {
    targetRelation = opts.orm.model(opts.criteria.from);
  }
  else if (opts.criteria.junction && opts.orm) {
    targetRelation = opts.orm.junction(opts.criteria.junction);
  }
  ...
  this.heap = this.cache = new QueryHeap({
    ...
    footprint: false // (footprint === whether to fetch only primary keys + sort vectors, then do a final, extra query for each relation involved to get the rest of the attribute values)
  });
