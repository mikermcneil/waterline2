...
module.exports = function run (cb) {
  ...
  if (!this.worker) {
    ...
    // will always override the implicit one.
      ...
      // In compatibility mode, if this query has joins, the worker
      // should call the `_joinRaw` adapter bridge.  Also, the query
      // should be flagged as `raw`.
      if (this.orm && this.orm.compatiblityMode) {
        if (this.operations.joins && this.operations.joins.length) {
          this.raw = true;
          this.worker = function adapterWorkerFn(cb) {
