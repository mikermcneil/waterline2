...
module.exports = function run (cb) {
  ...
  var srcIdentity = criteria.from || criteria.junction;

  // If a `worker` is not defined, and it can't be infered,
  // we must send back a usage error.
  if (!this.worker) {
    if (!this.orm) {
      cb(new WLUsageError(require('util').format('Cannot run Query on relation: "%s" - either a `worker` function or valid `orm` instance must be provided', srcIdentity)));
      return;
    }
    ...
    // will always override the implicit one.
      ...
      relation = criteria.from ? query.orm.model(criteria.from) : (criteria.junction ? query.orm.junction(criteria.junction) : null);
      ...
      if (!adapter) {
        ...
        cb(new WLUsageError(require('util').format('Cannot run Query on relation: "%s" - its adapter is not specified or invalid`', srcIdentity)));
        return;
  ...
  if (this.raw) {
    this.worker(function (err, results) {
      if (_.isArray(results)) {
        ...
        query.heap.push(criteria.from||criteria.junction, results);
      }
  ...
  // run the query engine and integrator.
    ...
    this.heap.on('batch', function (relationIdentity, someRecords) {
      ...
      if (relationIdentity !== srcIdentity) {
        return;
      }
