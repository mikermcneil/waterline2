...
var keysIn = require('root-require')('standalone/keysIn');
...
function normalizeCriteria (criteriaTree, targetModel, criteriaMetadata) {
  ...
  return _.reduce(criteriaTree, function (memo, sub, key) {
    ...
    if (key === 'where') {
      memo.where = normalizeWhereTree(sub, targetModel, criteriaMetadata);
    }
    else if (key === 'select') {
      memo.select = normalizeSelectTree(sub, targetModel, criteriaMetadata);
    }
    else {
      memo[key] = sub;
    }
 ...
 * @return {Object|false}
 */
function normalizeWhereTree (whereTree, targetModel, criteriaMetadata) {
  ...
  // build the appropriate criteria object.
    ...
    // TODO:
    // if no tagetModel is available, emit a warning explaining what happened
  ...
  if (whereTree.like) {
    ...
    // and attach it to every attribute as a %SQL LIKE%-style criteria.
      ...
      // Expand `like` using the targetModel's schema if possible.
      if (targetModel) {

        // Add a LIKE subattribute modifier object for each non-association attribute
        // in targetModel, then delete the
        _.forEach(targetModel.attributes, function (attrDef, attrName) {
      ...
      // If no model is available, just ignore the top-level `like` entirely
      else {
  ...
  return _.reduce(whereTree, function (memo, sub, attrName) {
    ...
    if (sub.whose) {
      ...
      // Take recursive step
      sub.whose = normalizeWhereTree(sub.whose, subQueryModel, criteriaMetadata);
    }
 ...
 */
function normalizeSelectTree (selectTree, targetModel, criteriaMetadata) {
  ...
  // to the '*' syntax, which is special and different and means get **everything**, even
  // stuff that's not in the schema.
  //
  ...
  return _.reduce(selectTree, function (memo, sub, attrName) {
    ...
    sub = normalizeCriteria(sub, joinModel, criteriaMetadata);
