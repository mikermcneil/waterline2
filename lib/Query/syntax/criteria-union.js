...
module.exports = function criteriaUnion (x, y) {
  ...
  // Build conflicts set
  var conflicts = {};
  _(x).each(function eachAttrInX (sub, attrName) {
    conflicts[attrName] = conflicts[attrName] || sub;
  });
  _(y).each(function eachAttrInY (sub, attrName) {
    conflicts[attrName] = conflicts[attrName] || sub;
  });
  ...
  _.each(conflicts, function (conflict, attrName) {
    ...
    // Handle one-sided existence conflicts
    if (conflict.x && !conflict.y) {
      return;
    }
    else if (!conflict.x && conflict.y) {
      return;
    }
...
/**
 * Compute the INTERSECTION of two (flat) WHERE criteria objects.
 *
 * Errs on the side of permissiveness- better to net
 * more results than to skip some.
 *
 ...
 * @return {Criteria}
 */
// module.exports = function criteriaIntersection (x, y) {
