...
function Query (opts, worker) {
  ...
  // TODO: validate that the ORM instance was passed in
  this._orm = opts.orm;
  ...
  Query.super_.apply(this, [opts, function do_exec (cb) {
    ...
    // Don't use the query runner/engine if the `raw` is turned on:
    if (self._opts.raw) {
      if (!worker) return cb(null, []);
      else {
        return worker(function whenWorkerFinished (err, data){
