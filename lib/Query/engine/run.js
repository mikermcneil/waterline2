...
module.exports = function run (cb) {
  ...
  if (!this.worker) {
    ...
    // will always override the implicit one.
      ...
      // \/       an assertion inside of `Adapter.interpolate()`
      ...
      if (this.orm && this.orm.compatibilityMode) {
        ...
        var hasJoins = this.operations.joins && this.operations.joins.length;
        if (hasJoins && adapterSupportsJoinMethod) {
          // console.log('~~~~~> JOINS:', this.operations.joins);
          this.raw = true;
          this.preCombined = true; // (this flag means "skip the integrator")
          this.worker = function adapterWorkerFn(cb) {
            model._joinRaw(operations, cb);
          };
        }

        // Also, in compatibility mode, aggregation queries are always `raw` and `preCombined`
        if(this.operations.groupBy || this.operations.sum || this.operations.average || this.operations.min || this.operations.max) {
          // They also need `limit`, `skip`, and `select` removed
          delete this.operations.select;
          delete this.operations.limit;
          delete this.operations.skip;
          this.raw = true;
  ...
  if (this.raw) {
    this.worker(function (err, results) {
      if (_.isArray(results)) {
        ...
        self.heap.push(operations.from, results);
      }
