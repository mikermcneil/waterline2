      ...
      // (or use optimized approach, i.e. call `.join()` in the adapter)
        ...
        async.each(Object.keys(joins), function eachJoin (attrName, next_subop) {
          ...
          if (assoc.type === '1-->N') {
            ...
            // TODO: (optimization)
            // Merge in other WHERE clause from the subselect
            // (i.e. { select: { pets: { where: {name: 'fluffy'} } } })

            // Now take the recursive step and run the suboperation.
            doRecursiveStep(subop, function memfilter (subResults) {
              // Eliminate unnecessary subResults:
              // (includes intersection of subselect's where and any nested
              // subquery "whose" conditions which apply to this association)
              var subWhose = operation.where[attrName] && operation.where[attrName].whose;
              var subSelectWhere = operation.select[attrName] && operation.select[attrName].where;
              var inMemoryFilter = subWhose;
              console.log('context:', modelIdentity, 'operations', util.inspect(operation, false, null));
              console.log('Answering memfilter for "%s"',attrName);
              console.log('applying in-memory filter :', inMemoryFilter);
              console.log('on subResults :', subResults);
              subResults = WLFilter(subResults, {where: inMemoryFilter}).results;
              return subResults;
            }, next_subop);
            return;
          }
