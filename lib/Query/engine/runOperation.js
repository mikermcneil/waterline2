    ...
    .exec(function (err, _batchResults){
      ...
      // (or use optimized approach, i.e. call `.join()` in the adapter)
        ...
        async.each(Object.keys(joins), function eachJoin (attrName, next_subop) {
          ...
          // TODO: can probably remove this since `subselect`isn't being used anymore
          // ========================================
          ...
          function doRecursiveStep (subop, filter, doRecursiveStep_cb) {
            ...
            runOperation({
              ...
              // remove extra entries)
                ...
                log.skip('originalResultIds:', originalResultIds);
                ...
                if (assoc.type === '1-->N') {
                  ...
                  backfilteredBatchResults = _.where(filteredBatchResults, function (parentResult) {
                    log.skip('~~~~~~~If "'+modelIdentity+'"s "'+primaryKey+'" (',parentResult[primaryKey],') is in',filteredSubopResultIds, 'im keeping it (',_.contains(filteredSubopResultIds, parentResult[primaryKey]),')');
