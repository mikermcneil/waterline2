...
function runOperation (options, cb) {
  ...
  var flatWhere = flattenClause(operation.where);
  ...
  async.doUntil(function doWhat (next_batch) {
    ...
    .exec(function (err, _batchResults){
      ...
      if ( !Object.keys(joins).length ) {
        ...
        // localQueryHeap truly represent the entirety of one "level" of our recursive operationsTree parsing.
        //
  ...
  function afterwards(err) {
    ...
    // console.log('()()',operation.limit||30,'->',_.first(finalInCriteria, operation.limit||30));
    finalInCriteria = _.first(finalInCriteria, operation.limit||30);
