...
function criteriaCursor (options, cb) {
  ...
  // (TODO: When possible, use a more optimized approach)
  var pageNo;
  var batchResults;
      ...
      // (or use optimized approach, i.e. call `.join()` in the adapter)
        ...
        async.each(Object.keys(joins), function eachJoin (attrName, next_association) {
          ...
          if (assoc.type === '1-->N') {
            ...
            var subCriteria = rule.getChildCriteria(filteredBatchResults);
            var futureFilter = rule.getChildFilter(filteredBatchResults);
            ...
            // wtf?
            return next_association(new WLUsageError('Unrecognized association syntax'));
          }
