 ...
 *
 * @param {Object} opts
 *                 opts.criteria   - a branch of an ancestral criteria tree (or the entire tree, same thing)
 *                 opts.query      - reference to the parent Query that spawned this cursor
 *                 opts.filter     - in-memory filter to use on initial raw batch results (used for subqueries)
 *
 * @param {Function} cb
 */
...
function criteriaCursor (options, cb) {
  ...
  var modelIdentity = criteria.from;
  var srcModel = orm.model(modelIdentity);
  var primaryKey = srcModel.primaryKey;
  log.skip('***** criteriaCursor() on "%s" *****', modelIdentity);
  ...
  function afterwards(err) {
    ...
    finalInCriteria = WLFilter.sort(finalInCriteria, criteria.sort);
...
function normalizeAdapterOutput(adapterResults, orm) {
  ...
  if (!_.isArray(adapterResults)) {
    ...
    if (orm.compatibilityMode && _.isObject(adapterResults)) {
      ...
      // the name of the adapter, the database, the model, and the criteria in
