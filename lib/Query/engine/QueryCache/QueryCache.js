...
function QueryCache (opts) {
  ...
  if (!require('../../../ORM').isORM(this.orm)) {
    throw new WLUsageError('A valid `orm` instance is required when initializing a QueryCache');
  }
...
QueryCache.prototype.push = function (modelIdentity, newResults) {
  ...
  //
  this.emit('batch', modelIdentity, newResults);
  ////////////////////////////////////////////////////////////

  // Ensure an array exists for this model
  this._models[modelIdentity] = (this._models[modelIdentity] || []);
  var pk = this.orm.model(modelIdentity).primaryKey;
  var cachedResults = this._models[modelIdentity];

  // old simple way that doesn't check for duplicates:
  // this._models[modelIdentity] = (this._models[modelIdentity] || []).concat(newResults);

  _.each(newResults, function (newResult) {
    var criteria = {}; criteria[pk] = newResult[pk];
    var extantMatchingResult = _.findWhere(cachedResults, criteria);
    ...
    else cachedResults.push(newResult);
  });
