...
module.exports = function runner (operationsTree, orm) {
  ...
  var onStop = function (err) {
    ...
    else query.emit('done');
  };

  // Run operations
  runOperation(operationsTree, cache, onStop);

  return query;


  function runOperation (operation, cache, onStop) {
    var DEFAULT_LIMIT = 30;

    // The better way:

    // Find a batch of records (or use optimized approach)
      // For each batch of records:
        ...
        // Else:
          // Store results in cache (or store only the ids and values for the sort vector)
            // (if they don't fit w/i the limit, delete existing results which are worse matchse)
    ...
    // if (isWhereNested) {
    //   return onStop(new WLUsageError({reason:'Not implemented yet.'}));
    // }
