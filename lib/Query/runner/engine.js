      ...
      .exec(function (err, _batchResults){
        ...
        // (or use optimized approach, i.e. call `.join()` in the adapter)
          ...
          async.each(Object.keys(joins), function (attrName, next_subop) {
            ...
            //
            // (currently only address 1.N use case, where foreign keys
            // are stored on the parent side- TODO: support the rest)
            //
            ...
            subop.where = {};
            subop.where[primaryKey] = _.pluck(remainingResults, primaryKey);

            // console.log(subop.where);
