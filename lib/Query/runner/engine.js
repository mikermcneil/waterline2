  ...
  function runOperation (operation, cb) {
        ...
        // (or use optimized approach, i.e. call `.join()` in the adapter)
          ...
          async.each(Object.keys(joins), function (attrName, next_subop) {
            ...
            if (assoc.type === '1-->N') {
              ...
              subop.select[assoc.relation.primaryKey] = true;

              // Pluck the foreign key values from the original batch results
              // then strip records where a foreign key is undefined.
              fkValues = _.pluck(filteredBatchResults, assoc.foreignKeys[assoc.keeper]);
              fkValues = pruneUndefined(fkValues);
              ...
              pkValues = _.pluck(filteredBatchResults, primaryKey);
              pkValues = pruneUndefined(fkValues);

              // N.1 :: Foreign key is on child model.
              // Have subop find child records where:
              // child[fk] =IN= parents[pk]
              subop.where[assoc.foreignKeys[assoc.keeper.identity]] = pkValues;
              ...
              var junctionPk = assoc.junction.primaryKey;
            ...
            function doRecursiveStep (subop, doRecursiveStep_cb) {
              ...
              runOperation(subop, function (err, subopResults) {
                ...
                if (
                  ...
                  var originalResultIds = _.pluck(subopResults,assoc.relation.primaryKey);
                  // console.log('assoc.relation.primaryKey:', assoc.relation.primaryKey);
                  ...
                  var filteredSubopResults = WLFilter(subopResults, {where: subquery}).results;
                  // console.log('the filtered results:', filteredSubopResults);
                  var filteredSubopResultIds = _.pluck(filteredSubopResults, assoc.relation.primaryKey);
                  ...
                  // console.log('pushed:\n',backfilteredBatchResults,' to subop\'s ('+subop.from+') local QC:', localQueryCache);
                } // </if>
    ...
    function afterwards(err) {
      ...
      .exec(function (err, results) {
        ...
        // console.log('ids:',localQueryCache);
