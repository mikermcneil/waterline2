  ...
  function runOperation (operation, cb) {
    ...
    async.doUntil(function doWhat (next_batch) {
      ...
      .exec(function (err, _batchResults){
        ...
        // TODO: flatten nested WHEREs if necessary to make sure
          ...
          async.each(Object.keys(joins), function (attrName, next_subop) {
            ...
            if (attrDef.model) {
              ...
              runOperation(subop, function (err, results) {
                ...
                // to enforce subqueries (nested WHERE clauses) in-memory.
                ...
                if (results.length) {
                  ...
                  if (Object.keys(subqueries).length) {
                    ...
                    var filteredResults = WLFilter(results, subqueries).results;
                    ...
                    // the QueryCache.
                    var removeCriteria = _.intersection(
                      originalResultIds,
                      filteredResultIds
                    );
