...
module.exports = function _runner (operationsTree, parentQuery, orm) {


  // Grab hold of Query constructor (cant use require b/c of require loops)
  ...
  // Run operations
  runOperation(operationsTree, cache, onDone);

  return parentQuery;


  function runOperation (operation, cache, onDone) {
    ...
    // TODO:(or use optimized approach)
    ...
    async.doUntil(function doWhat (next) {
      ...
      // Pull out shallow WHERE
      // Pull out shallow SELECT
      // tODO
      ...
      .exec(function (err, _batchResults){
        ...
        // console.log('done: _batchResults:',_batchResults);

        // Expose `batchResults` for use in `until` predicate above.
        batchResults = _batchResults;

        // Run in-memory filter on batch.
        // TODO
        var remainingResults = _batchResults;


        // If the operation at the current cursor position has one or more
        // recursive WHEREs or SELECTs:
        var subqueries = extractSubOperations(operation, 'where');
        var joins = extractSubOperations(operation, 'select');


        if (Object.keys(subqueries).length || Object.keys(joins).length) {

          // Take the recursive step using the remaining child records
          // (or use optimized approach, e.g. `.join()` or `.findWhose()`)

          // TODO:

          // async.each(Object.keys(joins), function (attrName, cb) {
          //   var subop = joins[attrName];
          //   runOperation(subop, cache, function (err) {
          //     cb(err);
          //   });
          // }, onDone);
          // async.each(Object.keys(subqueries), function (attrName, cb) {
          //   var subop = joins[attrName];
          //   runOperation(subop, cache, function (err) {
          //     cb(err);
          //   });
          ...
          // Store results in cache (or store only the ids and values for the sort vector)
          // (if they don't fit w/i the limit, delete existing results which are worse matches)
    ...
    function afterwards() {
      ...
      // Run a final IN query using the cached ids and
      // sort vector values to replace our cached ids
      // with the complete result set.
      var inCriteria = { where: {} };
 ...
 * @return {Boolean} if the specified `targetKey` is a "subobject"
 */
function getIsSubObject(operation, targetKey) {
  var target = operation[targetKey];
  return typeof target === 'object';
}

/**
 * [extractSubOperations description]
 * @param  {[type]} operation [description]
 * @param  {[type]} targetKey [description]
 * @return {[type]}           [description]
 */
function extractSubOperations(operation, targetKey) {
  return _.reduce(operation.where, function (memo, subop, attrName) {
    if ( typeof subop === 'object' ) {
      memo[attrName] = subop;
    }
    return memo;
  }, {});
