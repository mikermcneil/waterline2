...
module.exports = function _runner (operationsTree, parentQuery, orm) {
  ...
  var onDone = function (err) {
    ...
    else parentQuery.emit('runner_finished');
  };
  ...
  function runOperation (operation, cache, onDone) {
    ...
    async.doUntil(function doWhat (next) {
      ...
      .exec(function (err, _batchResults){
        ...
        // If the query has one or more recursive WHEREs or SELECTs at the childs' depth:
        if (false) {
          ...
          cache.set(modelIdentity, remainingResults);
    ...
    function afterwards() {
      // Run a final IN query using the cached ids and sort vector values to get the complete results.
