...
function ORM (opts) {
  ...
  _.defaults(opts, {
    ...
    defaultAdapter: new Adapter({
      identity: '_default-adapter'
    }),
    // Unless otherwise specified, hook up models to this datastore.
    defaultDatastore: new Datastore({
      identity: '_default-datastore'
    }),
...
// Build atomic models and/or datastores in order to perform
...
// Migrate the physical collections in the ORM's datastores
// to make them match the current in-memory ontology.
