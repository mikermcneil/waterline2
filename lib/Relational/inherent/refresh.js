...
module.exports = function refresh () {
  ...
  if (!orm) {
    throw new WLUsageError(util.format('Relational "%s" is not attached to an ORM', identity));
  }
  ...
  this.associations = _.reduce(this.attributes, function (memo, attrDef, attrName) {
    if (_.isObject(attrDef) && attrDef.model || attrDef.collection) {
      ...
      // Two-way "belongsToMany" association
        ...
        relatedRelational.attributes[attrDef.via] &&
        relatedRelational.attributes[attrDef.via].model
      ) {
        assoc.type = 'N<->1';
        assoc.cardinality = 'n';
        assoc.oneway = false;
        assoc.keeper = relatedRelational;
      ...
      // Two-way "hasMany" association
        ...
        attrDef.collection && attrDef.via &&
        relatedRelational.attributes[attrDef.via] &&
        relatedRelational.attributes[attrDef.via].collection &&
        relatedRelational.attributes[attrDef.via].via === attrName
      ) {
