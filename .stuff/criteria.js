...
function normalizeCriteria (criteriaTree, targetModel, flags) {
  ...
  var criteriaModifiers = _.where(Object.keys(criteriaTree), doesKeyMatch($$.CRITERIA_MODS));
...
function normalizeWhereTree (whereTree, targetModel, flags) {
  ...
  var criteriaModifiers = _.where(Object.keys(whereTree), doesKeyMatch($$.CRITERIA_MODS));
  if (criteriaModifiers.length) {
  ...
  return _.reduce(whereTree, function (memo, sub, attrName) {
    ...
    var criteriaModifiers = _(subKeys)
      .where      ( doesKeyMatch($$.CRITERIA_MODS) )
      .difference ( Object.keys($$.SUBQUERY_MODS) )
      .valueOf();
    if (criteriaModifiers.length) {
    ...
    var subqueryModifiers = _.where(Object.keys(sub), doesKeyMatch($$.SUBQUERY_MODS));
...
function normalizeSelectTree (selectTree, targetModel, flags) {
  ...
  // Check if this level of the tree contains any top-level criteria modifiers
  var criteriaModifiers = _.where(Object.keys(selectTree), doesKeyMatch($$.CRITERIA_MODS));
  ...
  return _.reduce(selectTree, function (memo, sub, attrName) {
    ...
    // Check if the subtree contains any top-level criteria modifiers
    var criteriaModifiers = _.where(Object.keys(sub), doesKeyMatch($$.CRITERIA_MODS));
...
/**
 * Configure a case-insensitive check to see if
 * the which keys in an object are recognized `synonyms`.
 *
 * @param  {Object} synonyms [description]
 * @return {Function}
 */
